...
$q1 = ("SELECT * FROM empresa where estado=1");
            ...
            $cantidadEmpresa=$buscarEmpresa->rowCount();
...
<html>
<head>
	<title>Archivos - <?php echo $_SESSION['nombre']; ?></title>
...
">
        ...
        <section id="content">
          ...
          <div class="container">
                <div class="col s16">
                  <div class="row">
                    ...
                    <div class="col s4 m4 l4" style="margin-top: 25px;">
                    <a class="waves-effect waves-light btn-small pink lighten-1 " href="#requisitosAqui"><i class="material-icons left">arrow_downward</i>Requisitos</a>
                    </div>
                  </div>
            ...
            <div class="row">
              <?php 
                    while ($row1=$buscarEmpresa->fetch(PDO::FETCH_ASSOC)){ 
                      ...
                      $q2 = ("SELECT * FROM requisitosOficina");
                      $buscarRequisitos = $dbConn->prepare($q2);
                      $buscarRequisitos->execute();
                      $cantidadRequisitos=$buscarRequisitos->rowCount();
                      @$cont+=1;
               ...
               <div class="col s12 m4 l4">
                  <ul id="task-card" class="collection with-header">
                    <li class="collection-header blue darken-3">
                      <h4 class="task-card-title"><?php echo substr($row1['razonSocial'],0,15); ?></h4>
                      <p class="task-card-date"><?php echo $row1['nombreCliente']; ?></p>
                    ...
                    <?php while ($row2=$buscarRequisitos->fetch(PDO::FETCH_ASSOC)){

                   $q3 = ("SELECT * FROM archivo where idEmpresa=:idEmpresa and idRequisito=:idRequisito and estado=1 ");
                      $registroArchivo1 = $dbConn->prepare($q3);
                      $registroArchivo1->bindParam(':idEmpresa', $row1['idempresa'], PDO::PARAM_INT);
                      $registroArchivo1->bindParam(':idRequisito', $row2['idRequisito'], PDO::PARAM_INT);
                    ...
                    <li class="collection-item dismissable">
                      <input type="checkbox" id="<?php echo $row2['requisito'].$row1['idempresa']; ?>" name="<?php echo 'task'.$row1['idempresa']; ?>"  <?php echo $checkbox; ?> onclick='activarDesactivar(<?php echo $row1['idempresa'].','.$row2['idRequisito'].',this.checked'; ?> );' />
                      <label for="<?php echo $row2['requisito'].$row1['idempresa']; ?>"><?php echo $row2['requisito']; ?></label>
                    </li>
                    <p id="cantidadEmpresa" style="display: none;"><?php echo $cont; ?></p>
                    
                    <script type="text/javascript">
                        var cantidad = $('cantidadEmpresa').text();

                      function activarDesactivar(idEmpresa,idRequisito,estado){
                          ...
                          ///funcion para insertar datos
                            ...
                            var evento=1;
                              ...
                              success:function(r2){
                                ...
                                if(estadoE==true){
                                  M.toast({html: 'Se ha Confirmado requisito. :)', classes: 'rounded'});
                                }else{
                                   M.toast({html: 'Se ha Desconfirmado requisito. :)', classes: 'rounded'});
                                }
          ...
          <div class="row" id="requisitosAqui">
             <div class="col s12 m12 l12">
                  <ul id="projects-collection" class="collection z-depth-1" style="margin-top: 30px;">
                    <li class="collection-item avatar">
                      <i class="material-icons cyan circle">archive</i>
                      <h6 class="collection-header m-0">Requisitos para optar officina</h6>
                      <p class="col s5 m5 l5">Listado</p>
                      <div class="col s6 m6 l6"><a class="btn-floating btn-large waves-effect cyan lighten-2 modal-trigger" href="#modal1"><i class="material-icons" onclick="modalArchivo();">add</i></a></div>
                    </li>
...
<!-- Modal para insertar inicio -->
                      <div id="modal1" class="modal">
                        <div class="modal-content">
                          <div class="col s12 m12 l12">
                            <h5>Insertar Nuevo Requisito</h5>
                        <form id="insertarRequisito" >
                          <input type="text" name="accionEjecutar" id="accionEjecutar" value="1" style="display: none;">
                           <div class="row">
                             <div class="input-field col s12 m12 l12 ">
                              <input id="txtRequisito" name="txtRequisito" type="text">
                      ...
                      <div id="modal2" class="modal">
                        ...
                        <form id="actualizarArchivo" >
                          ...
                          <input type="text" name="accionEjecutar" id="accionEjecutar" value="2" style="display: none;">
                           <div class="row">
                             <div class="input-field col s12 m12 l12 ">
                              <input id="modRequisito" name="modRequisito" type="text" autofocus>
                              <label for="first_name">Requisito</label>
                            </div>
                          </div>
                          <div class="row">
...
//uri general para todos los procesos
    ...
    function tablaArchivo(){
      var tabla2= $.ajax({
        url: uri1+"controller/archivoC.php",
        dataType: "text",
        async: false
    ...
    function actualizarDatos(clicked_id){
     ...
     idRequisito= $("#idRequisitos"+idModificar).val();
     requisito= $("#requisito"+idModificar).val();
     descripcion= $("#descripcion"+idModificar).val();
     //alert(requisito+" "+descripcion);
     //rellenar formulario para editar
     $("#idActualizar").val(idRequisito); 
     $("#modRequisito").val(requisito);
    ...
    function insertarDatos11(){
      ...
      var datosGuardar= $("#insertarRequisito").serialize();
      //alert(datosGuardar);
      $.ajax({
        type: "POST",
        url: uri1+"controller/archivoC.php",
    ...
    function actualizaDatos(){
      ...
      var datosGuardar= $("#actualizarArchivo").serialize();
      //alert(datosGuardar);
      $.ajax({
        type: "POST",
        url: uri1+"controller/archivoC.php",
